#!/usr/bin/env python3
"""Fix owk-005 PR - reset branch, copy files, recommit."""
import subprocess, os, shutil

REPO = r"C:\Users\rmalk\projects\owockibot-bounty-sync-"
SRC = r"C:\Users\rmalk\projects\razel369-aia\owk-005-razel"
...
for f in os.listdir(SRC):
    print(f"  {f}: {os.path.getsize(os.path.join(SRC, f))}b")

# 2) Close the empty PR #38
rc, out, err = run(["gh","pr","close","38","--repo","cyrilawoyemi99-max/owockibot-bounty-sync-","--delete-branch"])
print(f"\nclose PR #38: rc={rc}, err={err[:200]}")

# 3) Go back to main
rc, out, err = run(["git","-C",REPO,"checkout","main"])
print(f"checkout main: rc={rc}, err={err[:200]}")

# 4) Delete the owk-005-razel-audit branch
rc, out, err = run(["git","-C",REPO,"branch","-D","owk-005-razel-audit"])
print(f"delete branch: rc={rc}, err={err[:200]}")

# 5) Create a new branch from main
rc, out, err = run(["git","-C",REPO,"checkout","-b","owk-005-razel-audit","origin/main"])
print(f"new branch from main: rc={rc}, err={err[:200]}")

# 6) Copy source files to the repo
DEST = os.path.join(REPO, "owk-005-razel")
os.makedirs(DEST, exist_ok=True)
for f in os.listdir(SRC):
    src = os.path.join(SRC, f)
    dst = os.path.join(DEST, f)
    shutil.copy2(src, dst)
    print(f"  copied {f}: {os.path.getsize(dst)}b")
...
Deliverable:
- OWK-005-SECURITY-AUDIT.md (8 findings: 1 Critical, 3 High, 2 Medium, 2 Low)
...
- `owk-005-razel/OWK-005-SECURITY-AUDIT.md` — the audit, 8 findings: 1 Critical, 3 High, 2 Medium, 2 Low
- `owk-005-razel/validate-evidence.py` — Python stdlib validator, re-fetches each cited source file and confirms every line number reference still resolves to the same line in the live main branch
- `owk-005-razel/submission-note.md` — context and validator output
- `owk-005-razel/README.md` — index
...
- **F8 (Low)** — rate-limit window is a fixed 1-minute bucket, not a sliding window; an attacker can fire 2x the limit by timing the bucket reset.
